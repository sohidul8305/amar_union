...
const River_erosion = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // ভ্যালিডেশন
    const required = ['name', 'fatherName', 'village', 'postOffice', 'upazila', 'district', 'nid', 'mobile', 'email', 'landDetails', 'lossDetails', 'issueDate'];
    ...
    const confirmResult = await Swal.fire({
      ...
      text: 'আপনার নদী ভাঙনের সনদের আবেদন জমা দিতে চান?',
      icon: 'question',
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto bg-white p-6 md:p-12 shadow-2xl rounded-lg border-t-8 border-blue-600">
        <h1 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 mb-8 uppercase">নদী ভাঙনের সনদপত্র ফরম</h1>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-10">
            ...
            {/* জমির বিবরণ, ক্ষতির বিবরণ, ইস্যুর তারিখ */}
            <input type="text" name="landDetails" placeholder="জমির বিবরণ (দাগ/খতিয়ান)" value={formData.landDetails} onChange={handleChange} className="p-3 border rounded-lg w-full focus:ring-2 focus:ring-blue-600 outline-none sm:col-span-2" required />
            <input type="text" name="lossDetails" placeholder="নদী ভাঙনের বিবরণ" value={formData.lossDetails} onChange={handleChange} className="p-3 border rounded-lg w-full focus:ring-2 focus:ring-blue-600 outline-none sm:col-span-2" required />
          ...
          <div className="text-gray-700 space-y-4 border-t pt-8">
            <p className="text-base sm:text-lg">
              এই মর্মে প্রত্যয়ন করা যাচ্ছে যে, <strong>{formData.name || '........'}</strong>, পিতা: <strong>{formData.fatherName || '........'}</strong>, গ্রাম: <strong>{formData.village || '........'}</strong>, ডাকঘর: <strong>{formData.postOffice || '........'}</strong>, উপজেলা: <strong>{formData.upazila || '........'}</strong>, জেলা: <strong>{formData.district || '........'}</strong> অত্র এলাকার একজন স্থায়ী বাসিন্দা।
            </p>
