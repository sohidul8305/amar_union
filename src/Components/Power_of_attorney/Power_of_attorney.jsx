...
const Power_of_attorney = () => {
    const [formData, setFormData] = useState({
        ...
        // দাতা (Principal) - নতুন ফিল্ডসহ
        grantorName: '',
        grantorFatherName: '',    // নতুন
        grantorNid: '',
        grantorMobile: '',
        grantorVillage: '',       // নতুন (গ্রাম/মহল্লা)
        grantorPostOffice: '',    // নতুন
        grantorUpazila: '',       // নতুন
        grantorDistrict: '',      // নতুন
        // গ্রহীতা (Attorney)
        attorneyName: '',
        attorneyNid: '',
        attorneyMobile: '',
        // সম্পত্তির ঠিকানা (পুরনো village, ward, postCode)
        village: '',
        ward: '',
        postCode: '',
        email: '',
    ...
    const handleSubmit = async (e) => {
        ...
        // বেসিক ভ্যালিডেশন
        const required = [
            'deedNumber', 'deedDate', 'registryOffice', 'propertyDetails', 'purpose',
            'grantorName', 'grantorFatherName', 'grantorNid', 'grantorMobile',
            'grantorVillage', 'grantorPostOffice', 'grantorUpazila', 'grantorDistrict',
            'attorneyName', 'attorneyNid', 'attorneyMobile',
            'village', 'ward', 'postCode',
            'email', 'issueDate'
        ];
                ...
                try {
                    ...
                    if (data.success) {
                        Swal.fire({
                            ...
                            html: `<strong>আবেদন আইডি: ${data.poaId}</strong>`,
                            confirmButtonColor: '#000F9F'
                        });
                        // ফর্ম রিসেট
                        setFormData({
            ...
            <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="p-6 md:p-10 space-y-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">দাতার নাম <span className="text-red-500">*</span></label>
                                <input required type="text" name="grantorName" value={formData.grantorName} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">পিতার নাম <span className="text-red-500">*</span></label>
                                <input required type="text" name="grantorFatherName" value={formData.grantorFatherName} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">দাতার NID নম্বর <span className="text-red-500">*</span></label>
                                <input required type="number" name="grantorNid" value={formData.grantorNid} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">দাতার মোবাইল নম্বর <span className="text-red-500">*</span></label>
                                <input required type="tel" name="grantorMobile" value={formData.grantorMobile} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="01XXXXXXXXX" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">গ্রাম/মহল্লা <span className="text-red-500">*</span></label>
                                <input required type="text" name="grantorVillage" value={formData.grantorVillage} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">ডাকঘর <span className="text-red-500">*</span></label>
                                <input required type="text" name="grantorPostOffice" value={formData.grantorPostOffice} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">উপজেলা <span className="text-red-500">*</span></label>
                                <input required type="text" name="grantorUpazila" value={formData.grantorUpazila} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">জেলা <span className="text-red-500">*</span></label>
                                <input required type="text" name="grantorDistrict" value={formData.grantorDistrict} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">গ্রহীতার নাম <span className="text-red-500">*</span></label>
                                <input required type="text" name="attorneyName" value={formData.attorneyName} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">গ্রহীতার NID নম্বর <span className="text-red-500">*</span></label>
                                <input required type="number" name="attorneyNid" value={formData.attorneyNid} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">গ্রহীতার মোবাইল নম্বর <span className="text-red-500">*</span></label>
                                <input required type="tel" name="attorneyMobile" value={formData.attorneyMobile} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="01XXXXXXXXX" />
                            </div>
                        </div>
                    </div>

                    {/* ===== সম্পত্তির বিবরণ (ঠিকানা সহ) ===== */}
                    <div className="space-y-4">
                        <h3 className="text-lg font-bold text-[#000F9F] flex items-center gap-2 border-b pb-2 border-gray-100">
                        ...
                        <div>
                            ...
                            <textarea required name="propertyDetails" value={formData.propertyDetails} onChange={handleChange} rows="3" className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F] resize-none" placeholder="উদা: জে.এল নং-১২, মৌজা- চকবাজার, খতিয়ান নং-৪৫০, দাগ নং-২৮০১..." />
                        </div>
                    ...
                    {/* ===== ইমেইল ও ইস্যুর তারিখ ===== */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-1">আবেদনকারীর ইমেইল <span className="text-red-500">*</span></label>
