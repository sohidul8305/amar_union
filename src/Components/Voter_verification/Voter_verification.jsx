...
const Voter_verification = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const required = ['name', 'fatherName', 'village', 'postOffice', 'upazila', 'district', 'nidNumber', 'mobile', 'email', 'voterNumber', 'area', 'issueDate'];
    ...
    try {
      const response = await fetch('https://amar-union-backend.vercel.app/api/voter-verification-certificate', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto bg-white p-6 md:p-12 shadow-2xl rounded-lg border-t-8 border-blue-600">
        <h1 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 mb-8 uppercase">ভোটার যাচাইকরণ ফরম</h1>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-10">
            ...
            {/* ভোটার নম্বর, এলাকা, মোবাইল, ইমেইল */}
          ...
          {/* Certificate/Result Preview - সম্পূর্ণ আপডেট */}
          <div className="text-gray-700 space-y-4 border-t pt-8">
            <p className="text-base sm:text-lg">
              এই মর্মে যাচাই করা যাচ্ছে যে, <strong>{formData.name || '........'}</strong>, পিতা: <strong>{formData.fatherName || '........'}</strong>, গ্রাম: <strong>{formData.village || '........'}</strong>, ডাকঘর: <strong>{formData.postOffice || '........'}</strong>, উপজেলা: <strong>{formData.upazila || '........'}</strong>, জেলা: <strong>{formData.district || '........'}</strong> অত্র এলাকার ভোটার তালিকার একজন অন্তর্ভুক্ত ব্যক্তি।
            </p>
            <p className="text-base sm:text-lg">
              তার এনআইডি নম্বর: <strong>{formData.nidNumber || '........'}</strong>, ভোটার নম্বর: <strong>{formData.voterNumber || '........'}</strong> এবং তিনি <strong>{formData.area || '........'}</strong> এলাকার ভোটার।
            </p>
