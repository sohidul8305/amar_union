...
const Religion_cert = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const required = ['name', 'fatherName', 'village', 'postOffice', 'upazila', 'district', 'nid', 'mobile', 'email', 'religion', 'issueDate'];
    ...
    const confirmResult = await Swal.fire({
      ...
      text: 'আপনার ধর্মীয় সনদের আবেদন জমা দিতে চান?',
      icon: 'question',
    ...
    try {
      const response = await fetch('https://amar-union-backend.vercel.app/api/religion-certificate', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto bg-white p-6 md:p-12 shadow-2xl rounded-lg border-t-8 border-blue-600">
        <h1 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 mb-8 uppercase">ধর্মীয় সনদপত্র ফরম</h1>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-10">
            ...
            {/* এনআইডি, মোবাইল, ইমেইল */}
            <input type="number" name="nid" placeholder="জাতীয় পরিচয়পত্র (NID)" value={formData.nid} onChange={handleChange} className="p-3 border rounded-lg w-full focus:ring-2 focus:ring-blue-600 outline-none" required />
            ...
            {/* ধর্ম ও ইস্যুর তারিখ */}
            <input type="text" name="religion" placeholder="ধর্মের নাম" value={formData.religion} onChange={handleChange} className="p-3 border rounded-lg w-full focus:ring-2 focus:ring-blue-600 outline-none" required />
            <input type="date" name="issueDate" value={formData.issueDate} onChange={handleChange} className="p-3 border rounded-lg w-full focus:ring-2 focus:ring-blue-600 outline-none" required />
          </div>
