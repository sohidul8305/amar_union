...
const Landless_certificate = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // বেসিক ভ্যালিডেশন
        const required = ['applicantName', 'fatherHusbandName', 'motherName', 'gender', 'dateOfBirth', 'nidOrBrn',
            'mobileNumber', 'profession', 'yearlyIncome', 'familyMembers',
            'village', 'postOffice', 'upazila', 'district', 'ward', 'postCode', 'email', 'issueDate'];
            ...
            if (result.isConfirmed) {
                ...
                try {
                    ...
                    if (data.success) {
                        Swal.fire({
                            icon: 'success',
                            title: 'ভূমিহীন সনদের আবেদনটি সফল হয়েছে!',
                            html: `<strong>আবেদন আইডি: ${data.landlessId}</strong>`,
                            confirmButtonColor: '#000F9F'
                        });
                        // ফর্ম রিসেট
                        setFormData({
                ...
                <form onSubmit={handleSubmit} className="p-6 md:p-10 space-y-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input required type="date" name="dateOfBirth" value={formData.dateOfBirth} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input required type="text" name="village" value={formData.village} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="গ্রাম বা মহল্লার নাম" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">ডাকঘর <span className="text-red-500">*</span></label>
                                <input required type="text" name="postOffice" value={formData.postOffice} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="ডাকঘরের নাম" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">উপজেলা <span className="text-red-500">*</span></label>
                                <input required type="text" name="upazila" value={formData.upazila} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="উপজেলার নাম" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">জেলা <span className="text-red-500">*</span></label>
                                <input required type="text" name="district" value={formData.district} onChange={handleChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="জেলার নাম" />
                            </div>
                            <div>
                    ...
                    <div className="flex justify-end pt-4">
                        <button type="submit" disabled={loading} className="w-full sm:w-auto bg-[#000F9F] text-white font-bold px-10 py-3.5 rounded-xl hover:bg-[#0015cc] shadow-md hover:shadow-lg transition-all duration-200 text-sm cursor-pointer disabled:opacity-50">
                            {loading ? 'সাবমিট হচ্ছে...' : 'ভূমিহীন সনদের আবেদন জমা দিন'}
                        </button>
                    </div>
                </form>
            </div>
