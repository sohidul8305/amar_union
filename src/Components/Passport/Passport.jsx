...
const Passport = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const required = ['name', 'fatherName', 'village', 'postOffice', 'upazila', 'district', 'nidNumber', 'mobile', 'email', 'issueDate'];
    ...
    const confirmResult = await Swal.fire({
      ...
      text: 'আপনার পাসপোর্ট সুপারিশপত্রের আবেদন জমা দিতে চান?',
      icon: 'question',
    ...
    try {
      const response = await fetch('https://amar-union-backend.vercel.app/api/passport-certificate', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto bg-white p-6 md:p-12 shadow-2xl rounded-lg border-t-8 border-blue-600">
        <h1 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 mb-8 uppercase">পাসপোর্ট সুপারিশপত্র ফরম</h1>

        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-10">
            {/* নাম ও পিতা */}
            <input type="text" name="name" placeholder="আবেদনকারীর নাম" value={formData.name} onChange={handleChange} className="p-3 border rounded-lg w-full focus:ring-2 focus:ring-blue-600 outline-none" required />
          ...
          <div className="text-gray-700 space-y-4 border-t pt-8">
            <p className="text-base sm:text-lg">
              এই মর্মে প্রত্যয়ন করা যাচ্ছে যে, <strong>{formData.name || '........'}</strong>, পিতা: <strong>{formData.fatherName || '........'}</strong>, গ্রাম: <strong>{formData.village || '........'}</strong>, ডাকঘর: <strong>{formData.postOffice || '........'}</strong>, উপজেলা: <strong>{formData.upazila || '........'}</strong>, জেলা: <strong>{formData.district || '........'}</strong> অত্র এলাকার একজন স্থায়ী বাসিন্দা।
            </p>
            <p className="text-base sm:text-lg">জাতীয় পরিচয়পত্র নং: <strong>{formData.nidNumber || '........'}</strong></p>
