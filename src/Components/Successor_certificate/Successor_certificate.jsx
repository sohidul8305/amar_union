...
const Successor_certificate = () => {
    // ----- মৃত ব্যক্তির তথ্য (ডাকঘর, উপজেলা, জেলা সহ) -----
    ...
    const addSuccessorRow = () => setSuccessors([
        ...successors,
        { name: '', relation: '', age: '', maritalStatus: 'বিবাহিত', nidBirth: '' }
    ]);
    ...
    const handleSubmit = async (e) => {
        ...
        // বেসিক ভ্যালিডেশন
        const requiredDeceased = ['name', 'fatherHusband', 'deathDate', 'nid', 'village', 'postOffice', 'upazila', 'district', 'ward', 'postCode'];
        const missingDeceased = requiredDeceased.filter(f => !deceasedInfo[f]);
        const requiredApplicant = ['name', 'fatherName', 'nid', 'mobile', 'email', 'village', 'postOffice', 'upazila', 'district', 'ward', 'postCode', 'issueDate'];
        const missingApplicant = requiredApplicant.filter(f => !applicantInfo[f]);

        if (missingDeceased.length > 0 || missingApplicant.length > 0) {
            Swal.fire({
                icon: 'error',
                title: 'ত্রুটি',
                text: 'দয়া করে সকল আবশ্যক (*) চিহ্নিত ফিল্ড পূরণ করুন।',
                confirmButtonColor: '#000F9F'
            });
            return;
        }
                ...
                try {
                    ...
                    if (data.success) {
                        Swal.fire({
                            ...
                            html: `<strong>আবেদন আইডি: ${data.successorId}</strong>`,
                            confirmButtonColor: '#000F9F'
                        });
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'দুঃখিত!',
                        text: 'আবেদনটি সাবমিট করা যায়নি।',
        ...
        <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8 font-sans">
            <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="p-6 md:p-10 space-y-8">
                    {/* ===== মৃত ব্যক্তির বিবরণ (ডাকঘর, উপজেলা, জেলা সহ) ===== */}
                    ...
                    {/* ===== মৃত ব্যক্তির শেষ ঠিকানা (ডাকঘর, উপজেলা, জেলা সহ) ===== */}
                    <div className="space-y-4">
                        <h3 className="text-lg font-bold text-[#000F9F] flex items-center gap-2 border-b pb-2 border-gray-100">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input required type="text" name="postOffice" value={deceasedInfo.postOffice} onChange={handleDeceasedChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">উপজেলা <span className="text-red-500">*</span></label>
                                <input required type="text" name="upazila" value={deceasedInfo.upazila} onChange={handleDeceasedChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">জেলা <span className="text-red-500">*</span></label>
                                <input required type="text" name="district" value={deceasedInfo.district} onChange={handleDeceasedChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                    ...
                    {/* ===== আবেদনকারীর বিবরণ (সব ফিল্ডসহ) ===== */}
                    <div className="space-y-4">
                        <h3 className="text-lg font-bold text-[#000F9F] flex items-center gap-2 border-b pb-2 border-gray-100">
                            <span>👤</span> আবেদনকারীর বিবরণ
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">আবেদনকারীর নাম <span className="text-red-500">*</span></label>
                                <input required type="text" name="name" value={applicantInfo.name} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">পিতার নাম <span className="text-red-500">*</span></label>
                                <input required type="text" name="fatherName" value={applicantInfo.fatherName} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">এনআইডি <span className="text-red-500">*</span></label>
                                <input required type="number" name="nid" value={applicantInfo.nid} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">মোবাইল <span className="text-red-500">*</span></label>
                                <input required type="tel" name="mobile" value={applicantInfo.mobile} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="01XXXXXXXXX" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">ইমেইল <span className="text-red-500">*</span></label>
                                <input required type="email" name="email" value={applicantInfo.email} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="example@mail.com" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">জন্ম  তারিখ <span className="text-red-500">*</span></label>
                                <input required type="date" name="issueDate" value={applicantInfo.issueDate} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <h3 className="text-lg font-bold text-[#000F9F] flex items-center gap-2 border-b pb-2 border-gray-100">
                            <span>📍</span> আবেদনকারীর ঠিকানা
                        </h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">গ্রাম/মহল্লা <span className="text-red-500">*</span></label>
                                <input required type="text" name="village" value={applicantInfo.village} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">ডাকঘর <span className="text-red-500">*</span></label>
                                <input required type="text" name="postOffice" value={applicantInfo.postOffice} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">উপজেলা <span className="text-red-500">*</span></label>
                                <input required type="text" name="upazila" value={applicantInfo.upazila} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">জেলা <span className="text-red-500">*</span></label>
                                <input required type="text" name="district" value={applicantInfo.district} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">ওয়ার্ড নং <span className="text-red-500">*</span></label>
                                <input required type="number" name="ward" value={applicantInfo.ward} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="১-৯" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">পোস্ট কোড <span className="text-red-500">*</span></label>
                                <input required type="number" name="postCode" value={applicantInfo.postCode} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                        </div>
                    </div>
