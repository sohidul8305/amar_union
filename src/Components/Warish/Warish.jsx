...
const Warish = () => {
    ...
    // ওয়ারিশদের ডাইনামিক তালিকা হ্যান্ডেল করার স্টেট
    ...
    // ফর্ম সাবমিট হ্যান্ডলার (Are you sure কনফার্মেশন ও সাকসেস অ্যালার্ট সহ)
            ...
            if (result.isConfirmed) {
                ...
                try {
                    const submissionData = {
                        ...
                        applicantInfo,
                        heirs,
                        deathCertificateDocName: files.deathCertificateDoc ? files.deathCertificateDoc.name : null,
                        applicantNidDocName: files.applicantNidDoc ? files.applicantNidDoc.name : null
                    };

                    const response = await fetch('http://localhost:5000/warish', {
                        method: 'POST',
                        headers: {
                    ...
                    if (data.success) {
                        // ২. সফলভাবে সেভ হলে সাকসেস মেসেজ ও ওয়ারিশ আইডি শো করা
                        Swal.fire({
                            icon: 'success',
                            title: 'আবেদনটি সফলভাবে গৃহীত হয়েছে!',
                            text: `আপনার ওয়ারিশ ট্র্যাকিং আইডি: ${data.warishId}। চেয়ারম্যান/কাউন্সিলর মহোদয়ের যাচাইকরণের পর সনদটি প্রস্তুত করা হবে।`,
                            confirmButtonText: 'ঠিক আছে',
                            confirmButtonColor: '#000F9F'
                        });

                        // সম্পূর্ণ স্টেট ও ফর্ম রিসেট
                        setDeceasedInfo({ deceasedName: '', fatherHusbandName: '', motherName: '', deathDate: '', nid: '', ward: '', village: '', holdingNo: '' });
                        setApplicantInfo({ applicantName: '', applicantNid: '', applicantMobile: '', relationWithDeceased: 'পুত্র' });
        ...
        <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8 font-sans">
            <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
                ...
                <form id="warish-form" onSubmit={handleSubmit} className="p-6 md:p-10 space-y-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input required type="text" name="deceasedName" value={deceasedInfo.deceasedName} onChange={handleDeceasedChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="উদা: মরহুম আলহাজ্ব করিম উল্লাহ" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">পিতা/স্বামীর নাম <span className="text-red-500">*</span></label>
                                <input required type="text" name="fatherHusbandName" value={deceasedInfo.fatherHusbandName} onChange={handleDeceasedChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">মাতার নাম <span className="text-red-500">*</span></label>
                                <input required type="text" name="motherName" value={deceasedInfo.motherName} onChange={handleDeceasedChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input required type="number" name="nid" value={deceasedInfo.nid} onChange={handleDeceasedChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div className="grid grid-cols-3 gap-2">
                                <div>
                                    <label className="block text-[11px] font-semibold text-gray-700 mb-1">ওয়ার্ড নং <span className="text-red-500">*</span></label>
                    ...
                    <div className="space-y-4">
                        <h3 className="text-lg font-bold text-[#000F9F] flex items-center gap-2 border-b pb-2 border-gray-100">
                            <span>👤</span> আবেদনকারীর বিবরণ (যিনি ওয়ারিশদের পক্ষে ফরম পূরণ করছেন)
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">আবেদনকারীর নাম <span className="text-red-500">*</span></label>
                                <input required type="text" name="applicantName" value={applicantInfo.applicantName} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="উদা: মোঃ শফিকুর রহমান" />
                            </div>
                            <div>
                                ...
                                <select name="relationWithDeceased" value={applicantInfo.relationWithDeceased} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm bg-white outline-none focus:ring-2 focus:ring-[#000F9F]">
                                    <option>পুত্র</option>
                                    <option>কন্যা</option>
                            ...
                            <div>
                                ...
                                <input required type="number" name="applicantNid" value={applicantInfo.applicantNid} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" />
                            </div>
                            <div>
                                ...
                                <input required type="tel" name="applicantMobile" value={applicantInfo.applicantMobile} onChange={handleApplicantChange} className="w-full border border-gray-300 p-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#000F9F]" placeholder="01XXXXXXXXX" />
                            </div>
                        </div>
                    </div>

                    {/* সেকশন ৩: ওয়ারিশদের ডাইনামিক তালিকা */}
                    <div className="space-y-4">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b pb-2 border-gray-100 gap-2">
                            <h3 className="text-lg font-bold text-[#000F9F] flex items-center gap-2">
                                <span>👨‍👩‍👧‍👦</span> জীবিত উত্তরাধিকারী (ওয়ারিশ) সমূহের তালিকা
                            </h3>
                            <button type="button" onClick={addHeirRow} className="text-xs bg-emerald-600 text-white font-bold px-3 py-2 rounded-lg hover:bg-emerald-700 transition-colors flex items-center gap-1 cursor-pointer w-fit">
                                <span>➕</span> নতুন ওয়ারিশ যোগ করুন
                            </button>
                        </div>
                            ...
                            {heirs.map((heir, index) => (
                                <div key={index} className="grid grid-cols-1 sm:grid-cols-12 gap-2 bg-slate-50 p-3 rounded-xl border border-gray-200 relative items-end">
                                    ...
                                    <div className="sm:col-span-3">
                                        <label className="block text-[11px] font-semibold text-gray-600 mb-1">ওয়ারিশের নাম</label>
                                    ...
                                    <div className="sm:col-span-1">
                                        ...
                                        <input required type="number" name="age" value={heir.age} onChange={(e) => handleHeirChange(index, e)} className="w-full bg-white border border-gray-300 p-2 rounded-lg text-xs outline-none focus:ring-1 focus:ring-[#000F9F]" placeholder="বয়স" />
                                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="border border-gray-200 p-4 rounded-xl bg-slate-50 flex items-center justify-between">
                                <div className="max-w-[60%]">
                                    <p className="text-xs font-bold text-gray-700 truncate">মৃত ব্যক্তির মৃত্যু সনদপত্র</p>
                                    <p className="text-[10px] text-gray-400 mt-0.5 truncate">{files.deathCertificateDoc ? files.deathCertificateDoc.name : "বাধ্যতামূলক ডকুমেন্ট"}</p>
                                </div>
                                <input required type="file" name="deathCertificateDoc" onChange={handleFileChange} accept="image/*,application/pdf" className="text-xs max-w-[180px]" />
                            </div>

                            <div className="border border-gray-200 p-4 rounded-xl bg-slate-50 flex items-center justify-between">
                                <div className="max-w-[60%]">
                                    <p className="text-xs font-bold text-gray-700 truncate">আবেদনকারীর জাতীয় পরিচয়পত্র (NID)</p>
                                    <p className="text-[10px] text-gray-400 mt-0.5 truncate">{files.applicantNidDoc ? files.applicantNidDoc.name : "বাধ্যতামূলক ডকুমেন্ট"}</p>
                                </div>
                                <input required type="file" name="applicantNidDoc" onChange={handleFileChange} accept="image/*,application/pdf" className="text-xs max-w-[180px]" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 p-4 rounded-xl">
                        ...
                        <label htmlFor="warish-terms" className="text-xs text-amber-900 font-medium leading-relaxed cursor-pointer">
                            আমি সজ্ঞানে ঘোষণা করছি যে, উপরে উল্লেখিত ওয়ারিশদের তালিকা সম্পূর্ণ সত্য এবং কোনো জীবিত উত্তরাধিকারীর নাম গোপন করা হয়নি। কোনো তথ্য গোপন করার ফলে ভবিষ্যতে কোনো আইনি জটিলতা তৈরি হলে তার সম্পূর্ণ দায় আমার।
                        </label>
                    </div>
